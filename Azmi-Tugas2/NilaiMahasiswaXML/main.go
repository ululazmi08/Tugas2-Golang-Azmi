...
func getNilai(w http.ResponseWriter, r *http.Request) {
	...
	for result.Next() {
		var mhs mahasiswa
		err := result.Scan(&mhs.NoBp, &mhs.Nama, &mhs.NoHp, &mhs.Alamat.Jalan, &mhs.Alamat.Kelurahan, &mhs.Alamat.Kecamatan, &mhs.Alamat.Kabupaten, &mhs.Alamat.Provinsi, &mhs.Fakultas, &mhs.Jurusan)
		...
		for resultNilai.Next() {
			var nilaik nilai
			err := resultNilai.Scan(&nilaik.NoBp, &nilaik.IDMatkul, &nilaik.NamaMatkul, &nilaik.Nilai, &nilaik.Semester)
			if err != nil {
				panic(err.Error())
			}
			mhs.Nilai = append(mhs.Nilai, nilaik)
		}
		mhsi = append(mhsi, mhs)
	}
	w.Write([]byte("<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"))
	xml.NewEncoder(w).Encode(mhsi)
}
...
func main() {
	...
	password := yamlConfig.Connection.Password
	database := yamlConfig.Connection.Database

	var (
		mySQL = fmt.Sprintf("%v:%v@tcp(%v:%v)/%v", user, password, host, port, database)
	)
	...
	r.HandleFunc("/mahasiswa/{NoBp}", getNilai).Methods("GET")
	r.HandleFunc("/mahasiswa", getNilaiAll).Methods("GET")
